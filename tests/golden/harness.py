...
class TradeGoldenClient(Protocol):
    def create_procurement(self, ctx, fixture): ...
    def create_sales_order(self, ctx, fixture): ...


class ErpGoldenClient(Protocol):
    def receive(self, ctx, purchase_order): ...
    def supplier_invoice(self, ctx, receipt): ...
    def pay_supplier(self, ctx, invoice): ...
    def ship(self, ctx, order): ...
    def customer_invoice(self, ctx, shipment): ...
    def receive_payment(self, ctx, invoice): ...
    def inventory_flow(self, ctx, fixture): ...
    def foreign_currency_flow(self, ctx, fixture): ...


class ReconciliationClient(Protocol):
    def assert_balanced(self, ctx, documents): ...
    def assert_inventory(self, ctx, movement): ...
    def assert_fx(self, ctx, transaction): ...


@dataclass(slots=True)
class GoldenHarness:
    trade: TradeGoldenClient
    erp: ErpGoldenClient
